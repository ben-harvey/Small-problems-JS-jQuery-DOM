/*
input: 1. search term, 2. string
output: string
rules:
  occurrenccs of search term are highlighted
  **CAPS**
algorithm:
  replace (capture groups)
*/

const searchWord = function (searchTerm, string) {
  const regex = new RegExp(`\\b${searchTerm}\\b`, 'gi');
  const newTerm = `**${searchTerm.toUpperCase()}**`
  return string.replace(regex, newTerm);
}
...
console.log(searchWord('rem', text));
// returns
